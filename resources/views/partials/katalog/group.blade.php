...
<div class="catalog-group-header">
    ...
    <div>
        <h3 class="catalog-group-title">Group {{ $meta['letter'] }}: {{ $meta['label'] }}</h3>
    </div>
</div>
...
<div class="catalog-items">
    @foreach ($items as $item)
        ...
        <a class="catalog-item" href="{{ route('training.show', ['training' => $item['slug']]) }}">
            <div>
                <div class="catalog-item-meta">
                    <span class="catalog-item-badge"> {{ $content['badge'] }} </span>
                </div>
                <h4 class="catalog-item-title">{{ $item['name'] }}</h4>
